...
def display(self):
    ...
    while current is not None:
        if current.coeff != 0:
            if current.exp == 0:
                result.append(f"{current.coeff}")
            elif current.exp == 1:
                result.append(f"{current.coeff}x")
            else:
...
def get_polynomial_input():
    ...
    print("Enter terms in descending order of exponents:")
    for _ in range(n):
...
if __name__ == "__main__":
    ...
    poly1 = get_polynomial_input()

    print("Enter Polynomial 2:")
    poly2 = get_polynomial_input()
